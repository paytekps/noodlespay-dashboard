...
export function EntitlementEditor({ merchantId, device, capabilities, initialEntitlements, token, canEdit }: { merchantId: string; device: TerminalDevice; capabilities: TerminalCapability[]; initialEntitlements: TerminalDashboardData['entitlements']; token: string; canEdit: boolean }) {
  ...
  return <div className="mt-4 border-t pt-4"><h4 className="text-sm font-semibold">Allowed features</h4><div className="mt-2 grid gap-2 sm:grid-cols-2">{capabilities.map(capability => { const compatible = capabilityWorksWithLayout(capability.key, layout); return <label key={capability.key} className={`flex items-center gap-2 rounded border p-2 text-sm ${compatible ? '' : 'bg-gray-50 text-gray-400'}`}><input type="checkbox" checked={enabled.has(capability.key)} disabled={!canEdit || !compatible || busy === capability.key} onChange={e => void toggle(capability.key, e.target.checked)} /><span>{capability.catalog_items[0]?.display_name ?? capability.key}</span><span className="ml-auto text-xs">{capability.scope}</span></label>; })}</div>{message && <div className="mt-2 text-xs text-gray-600">{message}</div>}</div>;
}
