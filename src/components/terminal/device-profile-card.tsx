...
import type { TerminalDevice, TerminalProfile } from '../../lib/gimml-terminal-dashboard/types';
...
export function DeviceProfileCard({ device, canConfigure, canEnroll, token, debugCertificateSha256 }: { device: TerminalDevice; canConfigure: boolean; canEnroll: boolean; token: string; debugCertificateSha256?: string }) {
  ...
  async function save() {
    setState('Saving…');
    const layoutKey = profile === 'GIMML_MINI' ? 'MINI' : 'ONE';
    const response = await fetch('/api/dashboard/terminal', { method: 'PUT', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ deviceId: device.id, profileKey: profile, layoutKey }) });
  ...
  return <article className="rounded-lg border p-4"><div className="flex flex-wrap justify-between gap-2"><div><div className="font-semibold">Serial {device.serial_number}</div><div className="text-xs text-gray-500">{device.enrollment_state} · revision {device.config_revision}</div></div><span className="rounded bg-gray-100 px-2 py-1 text-xs">{profileLabel(profile)}</span></div>
    {canConfigure ? <div className="mt-4"><label className="block max-w-sm text-sm font-medium">Assigned terminal type<select value={profile} onChange={e => setProfile(e.target.value as TerminalProfile)} className="mt-1 w-full rounded border bg-white p-2"><option value="GIMML_ONE">Gimml One</option><option value="GIMML_MINI">Gimml Mini</option></select></label><p className="mt-2 text-xs text-gray-500">The screen layout is set automatically from the terminal type. Gimml One uses the One layout; Gimml Mini uses the Mini layout.</p></div> : <div className="mt-4 rounded border bg-gray-50 p-3 text-sm"><div className="font-semibold">Assigned terminal: {profileLabel(profile)}</div><div className="mt-1 text-gray-600">This terminal type was ordered and programmed by the administrator. Contact support to request a different device type.</div></div>}
