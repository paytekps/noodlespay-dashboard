...
export default function MerchantForm({ salesRepresentatives, saving, onSubmit }: Props) {
  ...
  return <form onSubmit={submit} className="mt-8 space-y-7 rounded-xl border bg-white p-6 shadow-sm">
    ...
    </div><label className="mt-5 flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm"><input type="checkbox" checked={values.isTest} onChange={event => set('isTest',event.target.checked)} className="mt-0.5" /><span><strong>Test merchant</strong><span className="mt-1 block text-gray-700">Use only for a non-production account. A test merchant can be permanently deleted only while it has no transaction, settlement, or billing history.</span></span></label></fieldset>
