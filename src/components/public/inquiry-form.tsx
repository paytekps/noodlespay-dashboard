...
export default function InquiryForm({ kind, defaultPlan = 'basic' }: InquiryFormProps) {
  ...
  return <form onSubmit={submit} className="space-y-5 rounded-2xl border bg-white p-8 shadow-sm">
    ...
    {kind === 'order_request' && <><div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-medium">Plan<select name="plan" defaultValue={['basic','pro','premium'].includes(defaultPlan) ? defaultPlan : 'basic'} className="mt-1 w-full rounded-lg border px-3 py-2"><option value="basic">Basic</option><option value="pro">Pro</option><option value="premium">Premium</option></select></label><label className="text-sm font-medium">Number of devices<input required name="quantity" type="number" min="1" max="100" defaultValue="1" className="mt-1 w-full rounded-lg border px-3 py-2" /></label></div><div className="rounded-xl border border-emerald-200 bg-emerald-50 p-5"><div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-medium text-emerald-950">Merchant account<select required name="merchantAccountPreference" defaultValue="existing_account" className="mt-1 w-full rounded-lg border border-emerald-300 bg-white px-3 py-2 text-gray-950"><option value="existing_account">Use my existing merchant account</option><option value="need_merchant_account">I need help obtaining a merchant account</option></select></label><label className="text-sm font-medium text-emerald-950">Datecs processing network<select required name="processorPreference" defaultValue="" className="mt-1 w-full rounded-lg border border-emerald-300 bg-white px-3 py-2 text-gray-950"><option value="" disabled>Select a Datecs network</option><option value="fiserv_rapid_connect">Fiserv Rapid Connect</option><option value="tsys_sierra">TSYS / Global Payments — Sierra</option></select></label><label className="text-sm font-medium text-emerald-950 sm:col-span-2">Current processor or platform (optional)<input name="currentProcessorName" maxLength={120} placeholder="For example, Fiserv or TSYS" className="mt-1 w-full rounded-lg border border-emerald-300 bg-white px-3 py-2 text-gray-950" /></label></div><p className="mt-3 text-xs leading-5 text-emerald-900">Your merchant account remains yours. To work with this BlueCash-05, it must be boarded through either Fiserv Rapid Connect or TSYS / Global Payments — Sierra. We confirm that during setup.</p></div><label className="block text-sm font-medium">Shipping address<input name="shippingAddress" autoComplete="street-address" maxLength={250} className="mt-1 w-full rounded-lg border px-3 py-2" /></label><div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-medium">City<input name="shippingCity" autoComplete="address-level2" maxLength={120} className="mt-1 w-full rounded-lg border px-3 py-2" /></label><label className="text-sm font-medium">State / region<input name="shippingState" autoComplete="address-level1" maxLength={120} className="mt-1 w-full rounded-lg border px-3 py-2" /></label><label className="text-sm font-medium">Postal code<input name="shippingPostalCode" autoComplete="postal-code" maxLength={30} className="mt-1 w-full rounded-lg border px-3 py-2" /></label><label className="text-sm font-medium">Country<input name="shippingCountry" autoComplete="country-name" maxLength={80} defaultValue="United States" className="mt-1 w-full rounded-lg border px-3 py-2" /></label></div></>}
    ...
    {kind === 'order_request' && <p className="text-xs leading-5 text-gray-500">No payment is collected unless secure online checkout is configured with approved Gimml prices. Otherwise, this sends an order request for review.</p>}
  </form>;
}
