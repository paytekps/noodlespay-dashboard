...
type UserRow = { id: string; email: string | null; full_name: string | null; role: Role; merchant_id: string | null };
...
export default function UsersAndAccessPage() {
  ...
  async function inviteUser() {
    ...
    try {
      await callApi('POST', { email, fullName, role: newRole, merchantId: newRole === 'merchant' ? newMerchantId : null });
  ...
  async function saveUser(user: UserRow) {
    ...
    try {
      await callApi('PATCH', { userId: user.id, role: user.role, merchantId: user.role === 'merchant' ? user.merchant_id : null });
