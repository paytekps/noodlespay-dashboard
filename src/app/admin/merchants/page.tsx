...
import MerchantStatusConfirmation, { type MerchantStatusAction } from '../../../components/merchant-status-confirmation';
...
export default function MerchantsPage() {
  ...
  const [statusTarget, setStatusTarget] = useState<{ merchant: MerchantRecord; action: MerchantStatusAction } | null>(null);
  ...
  async function changeStatus() { if (!statusTarget) return; const { merchant, action } = statusTarget; setBusy(true); setError(''); setMessage(''); try { const payload = await request('PATCH', { merchantId: merchant.id, action }); const messages = { deactivate: `${merchant.name} and ${payload.affectedDevices ?? merchant.device_count} devices were deactivated.`, reactivate: `${merchant.name} was reactivated. Its devices remain inactive until individually approved.`, archive: `${merchant.name} was archived.` }; setMessage(messages[action]); setStatusTarget(null); } catch (cause) { setError(cause instanceof Error ? cause.message : 'Merchant status could not be changed.'); } finally { setBusy(false); } }
