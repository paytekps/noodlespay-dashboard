          ...
          <div className="flex flex-wrap items-center justify-between gap-3 bg-gray-900 px-4 py-3 text-white">
            <div className="flex flex-wrap items-center gap-1">
              ...
              {isMarketingPage ? <><Link className={navClass('/')} href="/">Home</Link><Link className={navClass('/how-it-works')} href="/how-it-works">How it works</Link><Link className={navClass('/pricing')} href="/pricing">Plans</Link><Link className={navClass('/contact')} href="/contact">Contact</Link></> : <>
                {(effectiveRole === 'super_admin' || effectiveRole === 'admin') && <><Link className={navClass('/admin')} href="/admin">Overview</Link><Link className={navClass('/dashboard/devices')} href="/dashboard/devices">Devices</Link><Link className={navClass('/dashboard/terminal')} href="/dashboard/terminal">Plans &amp; Features</Link><Link className={navClass('/transactions')} href="/transactions">Transactions</Link><Link className={navClass('/dashboard/settlements')} href="/dashboard/settlements">Batches</Link><Link className={navClass('/dashboard/integrations')} href="/dashboard/integrations">Integrations</Link><Link className={navClass('/admin/inquiries')} href="/admin/inquiries">Sales</Link>{effectiveRole === 'super_admin' && <><Link className={navClass('/admin/users')} href="/admin/users">Users</Link><Link className={navClass('/admin/permissions')} href="/admin/permissions">Permissions</Link></>}</>}
