...
export async function GET(req: Request) {
  ...
  const { data, error } = await context.admin.from('inquiries').select('id, created_at, updated_at, inquiry_type, status, full_name, email, phone, organization, plan, quantity, message, shipping_address, shipping_city, shipping_state, shipping_postal_code, shipping_country, admin_notes, payment_status').order('created_at', { ascending: false }).limit(500);
